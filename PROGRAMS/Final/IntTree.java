/**
 * 7. Write a method called isFull that returns true if a binary tree is full
 * and false if it is not. A full binary tree is one in which every node has 0
 * or 2 children. For example, reference trees #1 and #2 are not full, but #3
 * is full. By definition, the empty tree is considered full.
 * 
 * @author J.Ho
 */
...
public class IntTree {
	...
	public boolean isFull() {
		...
		return isFullCheck(overallRoot);
	}
	
	public boolean isFullCheck(IntTreeNode node) {
		...
		if(node == null){
	        ...
	        return isFullCheck(node.right) && isFullCheck(node.left);
	    } else {
	        return false;
	    }
	}
